...
打开 `/documents/upload`，选择最大 2 MB 的 UTF-8 `.txt`、`.md` 或文本型
`.pdf` 文件，点击“上传并写入知识库”。成功后页面会显示文档 ID、处理状态和
切片数量。PDF 必须包含可提取的文本层，当前不支持扫描件、加密文件或 OCR。
...
打开 `/chat` 输入问题。页面调用 `POST /answer/stream`，使用浏览器原生
`ReadableStream` 逐块追加模型答案，完成后展示来源文件、切片位置、PDF 来源页码、
相似度和实际引用内容。页面只展示最新一次问答，但会在新请求中携带内存里的最近
五轮完整对话，帮助后端理解指代和追问。只有完整结束的回答会进入历史；生成中断时
保留已经显示的部分答案并标记失败。历史只保存在页面内存中，刷新页面或点击“清空
对话”后消失。一次正常问答会产生真实 embedding 和聊天模型请求。
