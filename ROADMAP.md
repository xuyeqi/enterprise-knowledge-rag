...
- 前端全局头部品牌标题、Logo、主打产品图片和动效已经调整，等待生产构建与页面视觉验证。
- 下一项工作是规划并实现对话历史。
...
- 2026-07-14：将天然石产品陈列图作为本地构建资产展示在页面标题右侧，并增加产品标签、横向裁切、悬停动效和移动端布局；尚未执行 `npm run build` 或页面视觉验证。
