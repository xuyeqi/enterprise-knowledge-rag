...
- GitHub Actions 工作流固定放在 `.github/workflows/`，文件名表达用途。
- CI 只做可重复的离线质量检查，不连接真实 PostgreSQL、Redis 或百炼 API。
- 后端使用 `uv sync --locked --extra dev` 校验锁文件并安装依赖，再运行 `uv run pytest`。
- 前端使用 `npm ci` 严格按 `package-lock.json` 安装依赖，再运行 `npm run build`。
- CI 默认只需要 `contents: read` 权限，不写仓库、不发布和不部署。
